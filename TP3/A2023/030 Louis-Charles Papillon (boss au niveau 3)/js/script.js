...
function dessinerEnvahisseurs() {
    ...
    for (let n of envahisseurs) {
        ...
        dessinerObjet(n.x, n.y, n.largeur, n.hauteur, n.couleur);
...
function actualiserLeaderboard(newScore) {
    
    gHighScores.push(newScore);

    gHighScores.sort((a, b) => b - a);

    for (let i = 0; i < gLeaderboard.length; i++) {
        gLeaderboard[i].textContent = gHighScores[i];
    }
}
