...
function verifierFinDuJeu(envahisseurs) {
    ...
    while(indexEnvahisseur < envahisseurs.length){
        ...
        if ((envahisseur.hauteur + envahisseur.y) >= ZoneDeJeu.height){
...
function afficherFinDuJeu() {
    ...
    finalScoreElement.textContent = gScore;
...
let Tir1 = document.getElementById('sonTir1');
let Tir2 = document.getElementById('sonTir2');
let Tir3 = document.getElementById('sonTir3');
let Explosion1 = document.getElementById('sonExplosion1');
let Explosion2 = document.getElementById('sonExplosion2');
let Explosion3 = document.getElementById('sonExplosion3');
...
function ajusterVolumenMusique() {
    musique1.volume = controlVolumen.value;
    musique2.volume = controlVolumen.value;
}

function ajusterVolumenTir() {
    Tir1.volume = controlVolumen.value;
    Tir2.volume = controlVolumen.value;
    Tir3.volume = controlVolumen.value;
}

function ajusterVolumenExplosion() {
    Explosion1.volume = controlVolumen.value;
    Explosion2.volume = controlVolumen.value;
    Explosion3.volume = controlVolumen.value;
}
...
function dessinerObjet(positionX, positionY, largeur, hauteur, couleur) {
    ...
    const cuerpoWidth = largeur * 0.8;
    const cuerpoHeight = hauteur * 0.8;
    const alaWidth = largeur * 0.2;
    const alaHeight = hauteur * 0.4;
    const cabinaWidth = largeur * 0.6;
    const cabinaHeight = hauteur * 0.6;

    contexteZone.fillStyle = couleur;
    contexteZone.fillRect(positionX + largeur * 0.1, positionY + hauteur * 0.1, cuerpoWidth, cuerpoHeight);

    contexteZone.fillStyle = couleur;
    contexteZone.beginPath();
    contexteZone.moveTo(positionX, positionY + hauteur / 2);
    contexteZone.lineTo(positionX + alaWidth, positionY + hauteur / 2 - alaHeight / 2);
    contexteZone.lineTo(positionX + alaWidth, positionY + hauteur / 2 + alaHeight / 2);
    contexteZone.closePath();
    contexteZone.fill();
    contexteZone.beginPath();
    contexteZone.moveTo(positionX + largeur, positionY + hauteur / 2);
    contexteZone.lineTo(positionX + largeur - alaWidth, positionY + hauteur / 2 - alaHeight / 2);
    contexteZone.lineTo(positionX + largeur - alaWidth, positionY + hauteur / 2 + alaHeight / 2);
    contexteZone.closePath();
    contexteZone.fill();
    ...
    contexteZone.fillStyle = couleur;
    contexteZone.beginPath();
    contexteZone.ellipse(positionX + largeur / 2, positionY + hauteur / 2, cabinaWidth / 2, cabinaHeight / 2, 0, 0, Math.PI * 2);
    contexteZone.fill();
    contexteZone.strokeStyle = couleur;
    contexteZone.lineWidth = 2;
    contexteZone.stroke();

    contexteZone.fillStyle = couleur;
    contexteZone.fillRect(positionX + largeur * 0.1, positionY, cabinaWidth, cabinaHeight / 4); 
    contexteZone.fillRect(positionX + largeur * 0.1, positionY + hauteur - cabinaHeight / 4, cabinaWidth, cabinaHeight / 4); 
    contexteZone.fillRect(positionX + alaWidth / 2, positionY + hauteur / 2 - alaHeight / 4, alaWidth / 2, alaHeight / 4); 
    contexteZone.fillRect(positionX + largeur - alaWidth, positionY + hauteur / 2 - alaHeight / 4, alaWidth / 2, alaHeight / 4); 
    contexteZone.fillRect(positionX + alaWidth / 2, positionY + hauteur / 2 + alaHeight / 4, alaWidth / 2, alaHeight / 4); 
    contexteZone.fillRect(positionX + largeur - alaWidth, positionY + hauteur / 2 + alaHeight / 4, alaWidth / 2, alaHeight / 4); 
    contexteZone.fillRect(positionX + largeur / 2 - cabinaWidth / 4, positionY + hauteur / 2 - cabinaHeight / 4, cabinaWidth / 2, cabinaHeight / 2); 
}
