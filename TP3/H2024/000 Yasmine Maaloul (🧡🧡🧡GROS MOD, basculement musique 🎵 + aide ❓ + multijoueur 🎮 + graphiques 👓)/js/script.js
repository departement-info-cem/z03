...
function miseAJourJoueur1() {

   
    if (etatTouches.joueur1.gauche && joueur1.x > 0) {
        joueur1.x -= joueur1.vitesseDeplacement * modificateurVitesse;
    }
    if (etatTouches.joueur1.droite && joueur1.x + joueur1.largeur < ZoneDeJeu.width) {
        joueur1.x += joueur1.vitesseDeplacement * modificateurVitesse;
    }
...
function miseAJourJoueur2() {
    ...
    let modificateurVitesse = 1.0;
    if (etatTouches.joueur2.gauche && joueur2.x > 0) {
        joueur2.x -= joueur2.vitesseDeplacement * modificateurVitesse;
    }
    if (etatTouches.joueur2.droite && joueur2.x + joueur2.largeur < ZoneDeJeu.width) {
        joueur2.x += joueur2.vitesseDeplacement * modificateurVitesse;
    }
}
...
function dessinerJoueur1() {
    ...
    contexteZone.clearRect(joueur1.x, joueur1.y, joueur1.largeur, joueur1.hauteur);
    const playerImg = document.getElementById('playerImage');
    contexteZone.drawImage(playerImg, joueur1.x, joueur1.y, joueur1.largeur, joueur1.hauteur);
}
...
function dessinerJoueur2() {
    ...
    contexteZone.clearRect(joueur2.x, joueur2.y, joueur2.largeur, joueur2.hauteur);
    const player2Img = document.getElementById('player2Image');
    contexteZone.drawImage(player2Img, joueur2.x, joueur2.y, joueur2.largeur, joueur2.hauteur);
}
...
function dessinerEnvahisseurs() {
    ...
    // ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
    // Boucle
       for(let crouton of envahisseurs) {
        ...
        dessinerObjet(crouton.x, crouton.y, crouton.largeur, crouton.hauteur, crouton.couleur);
    }
...
function creerTir() {
    ...
    if (etatTouches.joueur1.tir && momentActuel - gMomentDernierTir >= gIntervalleEntreTirs) {
        // Créez un objet tir et ajoutez-le au tableau des tirs.
        tirs.push({
            x: joueur1.x + joueur1.largeur / 2 - TIR_LARGEUR / 2, 
            y: joueur1.y, 
  ...
  if (etatTouches.joueur2.tir && momentActuel - gMomentDernierTir >= gIntervalleEntreTirs) {
    
    tirs.push({
        x: joueur2.x + joueur2.largeur / 2 - TIR_LARGEUR / 2, 
        y: joueur2.y, 
        vitesseVerticale: VITESSE_DEPLACEMENT_TIR, 
        largeur: TIR_LARGEUR, 
        hauteur: TIR_HAUTEUR 
...
function gererTirs() {
    ...
    while (i >= 0) {
        ...
        //SI (condition)
        if (tirs.tirASupprimer == false && tirs.y < 0){
...
function redemarrerJeu() {
    ...
    joueur1.x = ZoneDeJeu.width / 4;
    joueur1.y = ZoneDeJeu.height - 70;
    

    joueur2.x = (ZoneDeJeu.width * 3) / 4;
    joueur2.y = ZoneDeJeu.height - 70;
...
let musiqueActive = localStorage.getItem("musiqueActive") === true;
let boutonMusique = document.getElementById("boutonMusique");
let iconeMusique = document.getElementById("iconeMusique");

function toggleMusique() {

    if (musiqueActive) {
        arreterSon(sonMusique);
        for (let i = 0; i < sonMusique.length; i++) {
            sonMusique[i].pause();
        }
        iconeMusique.src = "images/mic off.png";
    } else {
        if (sonMusique && sonMusique.length > 0) {
            jouerSonAleatoire(sonMusique);
        }
        iconeMusique.src = "images/mic on.png"; 
    }
   musiqueActive = !musiqueActive; 
   localStorage.setItem("musiqueActive", musiqueActive);
}
...
document.getElementById("aideBouton").addEventListener("click", showOverlay);
