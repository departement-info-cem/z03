function initialiserVague(envahisseurs) {
    ...
    gNiveauVague++;
    if (gNiveauVague % 3 == 0) {
        vagueBoss(envahisseurs);
    } else {
        vagueNormale(envahisseurs);
    }
}

function vagueBoss(envahisseurs) {
    ...
    const quantiteEnvahisseursActuelle = gNiveauVagueBoss;
    const vitesseActuelle = VITESSE_Y_MINIMALE + 2 * (gNiveauVague * INCREMENT_VITESSE);
    ...
    let ligne = 0;
    let i = 0;
    while (i < gNiveauVagueBoss) { //et si gNiveauVagueBoss > MAXIMUM_ENVAHISSEURS_PAR_LIGNE ?????
        ...
        envahisseurs.push({
            ...
            x: margeGauche + i * ESPACE_ENTRE_ENVAHISSEURS + AJUSTEMENT_POSITION_RAPIDE,

            y: DEBUT_Y_ENVAHISSEURS + ligne * DISTANCE_Y_ENTRE_LIGNES,

            largeur: TAILLE_ETROITE / 2,

            hauteur: HAUTEUR_ENVAHISSEUR,

            vitesseVerticale: vitesseActuelle * MULTIPLICATEUR_VITESSE_RAPIDE_VERTICALE * 0.5 * gNiveauVagueBoss,

            vitesseHorizontale: 5 * VITESSE_HORIZONTALE * gNiveauVagueBoss,
...
function vagueNormale(envahisseurs) {
