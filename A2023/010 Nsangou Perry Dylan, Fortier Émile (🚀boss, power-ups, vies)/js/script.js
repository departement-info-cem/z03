...
function miseAJourEnvahisseurs() {
    ...
    while (indexEnvahisseur < envahisseurs.length) {
        ...
        //MOD 1: retirer l'envahisseur lorsqu'il dépasse le tableau
...
function dessinerJoueur() {
    ...
    // ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
    //VOTRE CODE ICI
    dessinerObjet(joueur.x, joueur.y, joueur.largeur, joueur.hauteur, playerStarUpgraded ? COULEUR_JOUEUR_STAR : COULEUR_JOUEUR); 
}
...
function dessinerEnvahisseurs() {
    ...
    // Boucle
        ...
        for(let envahisseur of envahisseurs){
            if (envahisseur.is_special) {
                envahisseur.flash_count++;
                let couleur = envahisseur.couleur;
                if (envahisseur.flash_count >= 100) {
                    envahisseur.flash_count = 0;
                } else if (envahisseur.flash_count >= 60) {
                    couleur = "#ff1f40";
                } else if (envahisseur.flash_count >= 30) {
                    couleur = "#ffffff";
                }
                dessinerObjet(envahisseur.x, envahisseur.y, envahisseur.largeur, envahisseur.hauteur, couleur);
            } else {
...
function creerTir() {
    ...
    if (etatTouches.tir && momentActuel - gMomentDernierTir >= (playerStarUpgraded ? STAR_INTERVAL_TIR : gIntervalleEntreTirs)) {
...
function verifierCollisionTirEnvahisseurs(indexTir) {
    ...
    while (indexEnvahisseur >= 0) {
        ...
        if (collision(tirs[indexTir], envahisseurs[indexEnvahisseur])) {
            if (envahisseurs[indexEnvahisseur].is_boss) {
                jouerSonAleatoire(sonExplosion);
                creerExplosion(tirs[indexTir].x + tirs[indexTir].largeur / 2, tirs[indexTir].y);
                envahisseurs[indexEnvahisseur].hp -= 1;
                if (envahisseurs[indexEnvahisseur].hp <= 0) {
                    gScore += envahisseurs[indexEnvahisseur].points;
                    envahisseurs.splice(indexEnvahisseur, 1);
                }
                tirs.splice(indexTir, 1);
            } else {
                if (envahisseurs[indexEnvahisseur].is_special) {
                    start_star();
                }
...
function redemarrerJeu() {
    ...
    for (let i = 0; i < audio_elements.length; i++) {
        audio_elements[i].volume = SON_VOLUME;
    }
...
function afficherScore() {
    ...
    document.querySelector("#nbEnvahisseurs").textContent = "#Envahisseurs: " + envahisseurs.length;
       ...
       //MOD 1 : mise à jor du nombre de vies du joueur
       document.querySelector("#nbVies").textContent = "#Vies: " + gVies
}
...
function jouerSonAleatoire(listeSon) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (listeSon[indexAleatoire].paused) {
...
function pauseSon(listeSon) {
    for (let son of listeSon) {
        son.pause();
    }
}

function unPauseSon(listeSon) {
    for (let son of listeSon) {
...
function start_star() {
    playerStarUpgraded = true;
    pauseSon(sonMusique);
    jouerSonAleatoire(sonStar);
    setTimeout(end_star, STAR_DURATION * 1000);
}

function end_star() {
    if (playerStarUpgraded) {
        playerStarUpgraded = false;
        arreterSon(sonStar);
        unPauseSon(sonMusique);
    }
}
